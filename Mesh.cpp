	...
	: indexCount(indexCount), vertexCount(vertexCount)
{
	...
	//    be if we want the GPU to act on it (as in: draw it to the screen)
	{
		...
		vbd.ByteWidth = sizeof(Vertex) * vertexCount;       // Use the actual vertex count parameter
	...
	//    be if we want the GPU to act on it (as in: draw it to the screen)
	{
		...
		//  - Byte Width (uses actual index count vs. hardcoded count)
		//  - Bind Flag (used as an index buffer instead of a vertex buffer) 
		D3D11_BUFFER_DESC ibd = {};
		ibd.Usage = D3D11_USAGE_IMMUTABLE;	// Will NEVER change
		ibd.ByteWidth = sizeof(unsigned int) * indexCount;	// Use the actual index count parameter
...
Mesh::~Mesh()
{
	// prase smart pointers do this for us
}
...
// --------------------------------------------------------
// Sets buffers and draws this mesh
// --------------------------------------------------------
void Mesh::Draw()
{
